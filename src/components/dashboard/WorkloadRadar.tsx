  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-xl border border-zinc-800 bg-zinc-950 p-4 space-y-4">
          <div className="flex items-center justify-between text-xs">
            <span className="text-zinc-400">Interactive 14-Day Timeline</span>
            <div className="flex items-center gap-3 text-[11px] text-zinc-500">
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded bg-zinc-600" /> Normal
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded bg-zinc-300" /> High
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded bg-white" /> Crunch Day
              </span>
            </div>
          </div>

          {/* Bars Container */}
          <div className="grid grid-cols-14 gap-1.5 h-36 items-end pt-2 pb-1 border-b border-zinc-800">
            {days.map((day, idx) => {
              const isSelected = idx === selectedDayIndex;
              const barHeightPercent = Math.max(14, day.totalScore);
              
              let barColor = "bg-zinc-600 hover:bg-zinc-400";
              if (day.totalScore >= 70) {
                barColor = "bg-white hover:bg-zinc-200";
              } else if (day.totalScore >= 45) {
                barColor = "bg-zinc-300 hover:bg-white";
              }
                ...
                >
                  <div
                    className={`w-full rounded-t transition-all relative ${barColor} ${
                      isSelected ? "ring-2 ring-white ring-offset-2 ring-offset-black scale-y-105" : ""
                    }`}
                    style={{ height: `${barHeightPercent}%` }}
                  >
                    {day.isCrunchDay && (
                      <span className="absolute -top-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
                    )}
                  </div>
                  <span className={`text-[10px] font-mono leading-none ${isSelected ? "text-white font-bold" : "text-zinc-500"}`}>
          ...
          <div>
            ...
            <div className="mt-4 space-y-2 text-xs">
              ...
              <div className="flex items-center justify-between p-2 rounded bg-zinc-900 border border-zinc-800">
                ...
                <span className="font-mono text-white font-semibold">{peakDay?.deadlinesCount || 3} deadlines</span>
              </div>
              <div className="flex items-center justify-between p-2 rounded bg-zinc-900 border border-zinc-800">
