...
export function CommandPalette({ isOpen, onClose, onOpenBrainDump }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4 bg-black/70 backdrop-blur-sm animate-fade-in">
      ...
      <div className="relative w-full max-w-xl bg-nexus-900 border border-white/[0.12] rounded-xl shadow-2xl overflow-hidden z-10">
        <Command 
        ...
        >
          <div className="flex items-center px-4 border-b border-white/[0.08]">
            <Command.Input
              ...
              autoFocus
            />
            <span className="text-[10px] font-mono text-zinc-500 bg-white/[0.06] px-1.5 py-0.5 rounded border border-white/[0.08]">
              ESC
            </span>
          </div>
          ...
          <Command.List className="max-h-[380px] overflow-y-auto p-2 space-y-1">
            <Command.Empty className="py-6 text-center text-xs text-zinc-500">
              {search.startsWith("/") ? (
                <span>Press <kbd className="px-1 py-0.5 bg-white/10 rounded text-zinc-300">Enter</kbd> to run "{search}"</span>
              ) : (
            ...
            <Command.Group heading="Quick Actions">
              <Command.Item
                ...
                className="flex items-center gap-2.5 px-3 py-2 rounded-md text-xs text-zinc-300 hover:bg-white/[0.06] hover:text-white cursor-pointer transition-colors"
              >
            ...
            <Command.Group heading="Active Items">
              {items.slice(0, 5).map((item) => (
                <Command.Item
                  key={item.id}
                  onSelect={() => handleSelect(() => router.push("/tasks"))}
                  className="flex items-center justify-between px-3 py-2 rounded-md text-xs text-zinc-300 hover:bg-white/[0.06] hover:text-white cursor-pointer transition-colors"
                >
          ...
          <div className="px-4 py-2 bg-nexus-950/80 border-t border-white/[0.06] flex items-center justify-between text-[11px] text-zinc-500">
