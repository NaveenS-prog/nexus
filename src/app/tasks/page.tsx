...
export default function TasksPage() {
  ...
  const getSourceIcon = (source: Source) => {
    switch (source) {
      case "google_classroom": return <GraduationCap className="w-3.5 h-3.5 text-zinc-500" />;
      case "notion": return <Layers className="w-3.5 h-3.5 text-zinc-500" />;
      case "google_tasks": return <CheckSquare className="w-3.5 h-3.5 text-zinc-500" />;
      default: return <CheckSquare className="w-3.5 h-3.5 text-zinc-500" />;
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200/90 pb-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-zinc-950">Unified Tasks</h1>
          </div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full bg-white border border-zinc-200 rounded-lg pl-9 pr-3 py-1.5 text-xs text-zinc-900 placeholder-zinc-400 outline-none focus:border-zinc-950 shadow-xs"
          />
        </div>
      </div>

      {/* Quick Add Bar */}
      <form onSubmit={handleCreateTask} className="p-2.5 rounded-xl border border-zinc-200/90 bg-white/90 backdrop-blur-md shadow-xs flex flex-wrap items-center gap-2">
        <input
          type="text"
          value={newTitle}
          onChange={(e) => setNewTitle(e.target.value)}
          placeholder="Add a new task (e.g. 'Review OS Semaphore code')..."
          className="flex-1 min-w-[240px] bg-transparent px-3 py-1.5 text-xs text-zinc-900 placeholder-zinc-400 outline-none font-medium"
        />

        <select
          value={newCategory}
          onChange={(e) => setNewCategory(e.target.value as Category)}
          className="bg-zinc-50 border border-zinc-200 rounded-md px-2.5 py-1 text-xs text-zinc-700 outline-none"
        >
        ...
        <Button type="submit" size="sm" className="h-8 text-xs flex items-center gap-1.5 bg-zinc-950 text-white font-semibold hover:bg-zinc-800 shadow-sm">
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-1.5 border-b border-zinc-200/80 pb-2 overflow-x-auto text-xs">
        {[
      ...
      <div className="rounded-xl border border-zinc-200/90 bg-white/90 backdrop-blur-md divide-y divide-zinc-100 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.03),0_8px_24px_rgba(0,0,0,0.02)]">
        {filteredItems.length === 0 ? (
          <div className="p-12 text-center text-xs text-zinc-400">
            No tasks found in this view. Use the input above or press <kbd className="px-1.5 py-0.5 bg-zinc-100 border border-zinc-200 rounded text-zinc-600 font-mono">Ctrl+K</kbd> to add.
          </div>
        ) : (
          filteredItems.map((item) => {
            ...
            return (
              <div
                ...
                className={`p-3.5 flex items-center justify-between hover:bg-zinc-50/90 transition-all cursor-pointer group ${
              ...
              >
                <div className="flex items-center gap-3 min-w-0 pr-4">
                  <button
                    ...
                    className="flex-shrink-0 text-zinc-400 hover:text-zinc-950 transition-colors"
                  >
                    {isCompleted ? (
